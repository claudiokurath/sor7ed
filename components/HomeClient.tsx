...
export default function HomeClient() {
  return (
    <div className="bg-white text-black">
      ...
      {/* ── HERO ──────────────────────────────────────────── */}
      <section className="px-5 sm:px-8 md:px-12 pt-20 md:pt-24 pb-0 border-b-2 border-black">
      ...
      <section className="border-b-2 border-black">
        <div className="max-w-6xl mx-auto">
          <div className="px-5 sm:px-8 md:px-12 py-4 border-b border-black/10">
            <p className="label">What&apos;s happening right now? Tap to send on WhatsApp.</p>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3">
            {PROBLEMS.map((p, i) => (
              <a
                ...
                className={`group flex items-center justify-between px-5 sm:px-8 md:px-12 py-5 border-black/10 hover:bg-ps-yellow transition-colors ${
                  i < PROBLEMS.length - (PROBLEMS.length % 3 || 3) ? 'border-b' : ''
                } ${i % 3 !== 2 ? 'md:border-r' : ''} ${i % 2 !== 1 ? 'sm:border-r md:border-r-0' : ''}`}
              >
                <span className="font-display uppercase text-sm tracking-wide text-black group-hover:text-black">
                  {p.text}
                </span>
        ...
        <div className="max-w-6xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3">
            {[
              ...
              <div key={s.n} className={`px-5 sm:px-8 md:px-12 py-10 ${i < 2 ? 'md:border-r border-black/10' : ''}`}>
                <div className="font-display text-6xl text-black/10 mb-4 leading-none">{s.n}</div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4">
            {branches.map((branch, i) => (
              <Link
                key={branch.slug}
                href={`/${branch.slug}`}
                className={`group block px-5 sm:px-6 py-7 hover:bg-ps-yellow transition-colors border-black/10
                  ${i % 4 !== 3 ? 'md:border-r' : ''}
              ...
              >
                ...
                <h3 className="font-display uppercase text-sm tracking-wide text-black group-hover:text-black mb-1">
                  {branch.name}
                </h3>
                <p className="text-black/40 text-xs leading-relaxed line-clamp-2">{branch.description}</p>
              </Link>
            ))}
          </div>
        </div>
      ...
      {/* ── CTA ───────────────────────────────────────────── */}
      <section className="bg-ps-yellow">
        <div className="max-w-6xl mx-auto px-5 sm:px-8 md:px-12 py-16 flex flex-col md:flex-row md:items-center justify-between gap-8">
          <h2 className="font-display uppercase text-black leading-none" style={{ fontSize: 'clamp(2rem, 6vw, 4rem)' }}>
            Ready to get sorted?
          </h2>
          <div className="flex flex-wrap gap-3 shrink-0">
            <Link href="/tools" className="btn-primary">Start free →</Link>
