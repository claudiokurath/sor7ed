      ...
      <div>
        ...
        <section className="bg-black flex flex-col min-h-[100dvh]">
          ...
          <div className="px-6 md:px-12 pt-8 md:pt-24 pb-6 shrink-0 flex items-end justify-between gap-4">
            <motion.h1
              className="display-lg text-white leading-[0.9]"
              initial={{ opacity: 0, y: 20 }}
              animate={{ opacity: 1, y: 0 }}
              transition={{ duration: 0.4 }}
            >
              What&apos;s{" "}
              <span className="text-ps-yellow">stopping</span>{" "}
              <span className="text-transparent [-webkit-text-stroke:2px_#fff]">you?</span>
            </motion.h1>
            <motion.p
          ...
          {/* Problem tiles */}
          <nav
            className="flex-1 grid grid-cols-2 border-t-2 border-white/10"
            aria-label="Browse by problem"
          >
            {PROBLEMS.map((p, i) => (
              <motion.div
                key={p.text}
              ...
              >
                <Link
                  href={p.href}
                  className="group flex flex-col justify-between p-5 md:p-8 h-full 
                             hover:bg-ps-yellow active:bg-ps-yellow transition-colors duration-150"
                >
                  <div>
                    <p className="text-white/25 group-hover:text-black/40 text-[9px] font-display 
                                  uppercase tracking-[0.2em] mb-2 transition-colors">
                      {p.sub}
                    </p>
                    <span className="display-sm text-white group-hover:text-black transition-colors">
                      {p.text}
                    </span>
                  </div>
                  <span className="text-white/20 group-hover:text-black transition-colors self-end text-xl" 
                        aria-hidden="true">
                    →
                  </span>
                </Link>
          ...
          {/* Bottom bar */}
          <div className="shrink-0 border-t-2 border-white/10 px-6 md:px-12 py-3 
            ...
            <Link 
              href="/tools" 
              className="text-white/20 hover:text-white text-[9px] font-display 
        ...
        {featuredArticles.length > 0 && (
          <section className="bg-white border-b-2 border-black">
            <div className="max-w-6xl mx-auto">
              <div className="px-5 sm:px-8 md:px-12 py-4 border-b-2 border-black 
                              flex items-center justify-between">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3">
                {/* Hero article */}
                {hero && (
                  <div className="lg:col-span-2 border-b-2 lg:border-b-0 lg:border-r-2 
                                  border-black flex flex-col">
                    <div className="relative h-56 sm:h-72 lg:h-80 img-zoom border-b-2 border-black">
                      {hero.cover_image ? (
                        <Image
                        ...
                        <div className="h-full bg-black flex items-end p-8">
                    ...
                    <div className="flex flex-col flex-1 p-6 sm:p-8 md:p-10">
                      ...
                      <h2 className="display-md text-black leading-tight mb-4 flex-1">
                        {hero.title}
                      </h2>
                      ...
                      <div className="flex items-center justify-between border-t-2 border-black pt-5 mt-auto">
                        <Link
                          ...
                          className="text-[10px] font-display uppercase tracking-widest 
                                     text-black hover:text-ps-yellow transition-colors"
                        >
                          Read article →
                        </Link>
                        <SaveToPhoneButton
                ...
                <div className="flex flex-col">
                  {secondary.map((article, i) => (
                    ...
                    >
                      {article.cover_image && (
                        <div className="relative h-36 img-zoom border-b-2 border-black">
                          <Image
                      ...
                      <div className="flex flex-col flex-1 p-6">
                        ...
                        <h3 className="display-sm text-black leading-tight mb-2 flex-1">
                          {article.title}
                        </h3>
                  ...
                  {Array.from({ length: Math.max(0, 2 - secondary.length) }).map((_, i) => (
                    <div
                      key={`empty-${i}`}
                      className={`flex-1 flex items-center justify-center p-8 bg-black/[0.02]
                        ${secondary.length === 0 && i === 0 ? "border-b-2 border-black" : ""}`}
                    >
                      <Link href="/intelligence" className="label hover:text-black transition-colors">
                        Browse all articles →
                      </Link>
        ...
        <section className="bg-white border-b-2 border-black">
          <div className="max-w-6xl mx-auto">
            <div className="px-5 sm:px-8 md:px-12 py-4 border-b-2 border-black 
              ...
              <Link href="/explore" className="label hover:text-black transition-colors">
                Explore all →
              </Link>
            </div>
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4">
              {branches.map((branch, i) => (
                <Link
                  ...
                  `}
                >
                  <span className="text-2xl block mb-3" aria-hidden="true">{branch.icon}</span>
                  <p className="label mb-1 text-black/30 group-hover:text-white/30 transition-colors">
                    {branch.num}
                  </p>
                  <h3 className="font-display uppercase text-sm tracking-wide text-black 
                                 group-hover:text-white mb-1 transition-colors">
                    {branch.name}
                  </h3>
                  <p className="text-black/40 group-hover:text-white/40 text-xs 
                                leading-relaxed line-clamp-2 transition-colors">
                    {branch.description}
                  </p>
                </Link>
              ))}
          ...
          <div className="max-w-6xl mx-auto">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3">
              {HOW_IT_WORKS.map((s, i) => (
                <div
                  key={s.n}
                  className={`px-5 sm:px-8 md:px-12 py-10 
                    ${i < 2 ? "border-b md:border-b-0 md:border-r border-black/15" : ""}`}
                >
                  <div className="font-display text-6xl text-black/15 mb-4 leading-none" 
                       aria-hidden="true">
                    {s.n}
                  </div>
                  <h3 className="font-display uppercase text-base tracking-wide text-black mb-3">
                    {s.title}
                  </h3>
        ...
        <section className="bg-black">
          <div className="max-w-6xl mx-auto px-5 sm:px-8 md:px-12 py-16 
            ...
            <div>
              <h2 className="display-lg text-white leading-none mb-3">
            ...
            <div className="flex flex-wrap gap-3 shrink-0">
              ...
              <a
                ...
                aria-label="Text us on WhatsApp (opens in new tab)"
