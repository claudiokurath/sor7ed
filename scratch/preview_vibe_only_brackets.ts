...
function replaceTags(text: string, vibes: string[]): { updatedText: string; log: string[] } {
  const log: string[] = [];
...
async function main() {
  try {
    console.log("Simulating dynamic vibe-only style tag replacements...\n");
    let hasMore = true;
    ...
    while (hasMore) {
      ...
      for (const page of response.results) {
        ...
        const { log } = replaceTags(lyricsText, vibes);
        if (log.length > 0) {
          matchCount++;
          console.log(`Song #${matchCount}: "${title}"`);
          console.log(`  - Vibes: ${vibes.join(', ')}`);
          log.slice(0, 5).forEach(msg => console.log(`    * ${msg}`));
          if (log.length > 5) console.log(`    * ... and ${log.length - 5} more tags.`);
          console.log('--------------------------------------------------');
        }
      }
